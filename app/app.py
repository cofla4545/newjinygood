...
def start_recording():
    ...
    if not out.isOpened():
        ...
        return jsonify({'error': 'Could not open video writer.'}), 500
...
def stop_recording():
    ...
    try:
        ...
        audio_exists = os.path.exists(audio_out_path)
        print(f"Checking file existence: {video_out_path} exists: {video_exists}")
        print(f"Checking file existence: {audio_out_path} exists: {audio_exists}")
        if not video_exists or not audio_exists:
            return jsonify({'error': 'One of the output files does not exist.'}), 500
